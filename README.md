...
| [`passes/dce.py`](passes/dce.py) | Dead-code elimination via a single backward-liveness scan: drops any op whose outputs aren't transitively needed for a graph output, and prunes the now-unreferenced tensor shapes. A no-op on the current hand-built graph (verified), it's the cleanup that keeps the graph honest once an upstream pass leaves slack behind. |

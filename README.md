...
| [`passes/cse.py`](passes/cse.py) | Common subexpression elimination: merges structurally-identical ops (same kind, same rewritten inputs, same attrs), rewiring consumers and graph outputs to the canonical value. Processes in topological order rewriting inputs through a running remap, so dedup chains collapse. Conservative — never renames a graph output, purely structural equivalence. |
...
| [`codegen/llvm_kernels.py`](codegen/llvm_kernels.py) | LLVM codegen for actual **compute**, not just control flow: elementwise `add`/`mul`/`axpy` and a row-major **matmul** (`C[M,N] = A[M,K] @ B[K,N]`, three nested LLVM IR loops with the dot product accumulated in a phi register). Each is emitted as LLVM IR, JIT-compiled, and run on real float32 buffers via `ctypes` — checked against NumPy in [`tests/test_llvm_kernels.py`](tests/test_llvm_kernels.py). `llvm_op_overrides()` plugs the matmul into the executor so a whole decode step runs its GEMMs as native code. |
...
The compiler-technique pieces are real and independently testable: the IR, the fusion legality check, the analytic traffic-savings model, the shape-bucket specializer, and the LLVM codegen. The LLVM path is genuinely generated, verified, JIT-compiled, and executed — the branch-free *dispatch* classifier, elementwise *compute* kernels (`add`/`mul`/`axpy`), and a row-major **matmul** (three nested LLVM IR loops), all compiled to native code and run on real float32 buffers through `ctypes`. Via `execute_graph(..., op_overrides=llvm_op_overrides())`, an entire decode step runs its matmuls as native LLVM code and matches the NumPy backend within float32 tolerance — same IR, same passes, only the lowering changed.

The remaining ops (softmax, RoPE, the attention einsums) still execute on the NumPy reference backend (`fused_ops.py`) so the full pipeline runs without a GPU; that backend validates *correctness* — fusion never changes results — not GPU wall-clock performance, since CPU/NumPy doesn't have the HBM-bandwidth bottleneck that motivates kernel fusion in the first place. Lowering those ops too (or swapping in a Triton/CUTLASS backend behind the same `op_overrides` seam) — without touching the IR, fusion pass, or specializer — is the path to measured GPU numbers.
...
- [x] IR verifier (`passes/verify.py`), dead-code elimination (`passes/dce.py`, wired into the engine), and common subexpression elimination (`passes/cse.py`) passes; `Graph.to_dot()` Graphviz export

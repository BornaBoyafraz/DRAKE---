...
| [`codegen/llvm_kernels.py`](codegen/llvm_kernels.py) | LLVM codegen for actual **compute**, not just control flow: emits an LLVM IR loop (phi-node induction, `getelementptr`/load/store, fp ops) for elementwise `add`/`mul`/`axpy`, JIT-compiles it, and runs it on real float32 buffers via `ctypes` — verified bit-for-bit against NumPy in [`tests/test_llvm_kernels.py`](tests/test_llvm_kernels.py). The elementwise seed of a real kernel backend behind `KernelPlan`. |
...
The compiler-technique pieces are real and independently testable: the IR, the fusion legality check, the analytic traffic-savings model, the shape-bucket specializer, and the LLVM codegen. The LLVM path is genuinely generated, verified, JIT-compiled, and executed — both the branch-free *dispatch* classifier and, via [`codegen/llvm_kernels.py`](codegen/llvm_kernels.py), real elementwise *compute* kernels (`add`/`mul`/`axpy`): an LLVM IR loop (phi-node induction, `getelementptr`/load/store, fp ops) compiled to native code and run on real float32 buffers through `ctypes`, checked bit-for-bit against NumPy.

The heavier ops (matmul, attention) still execute on the NumPy reference backend (`fused_ops.py`) so the full pipeline runs without a GPU; that backend validates *correctness* — fusion never changes results — not GPU wall-clock performance, since CPU/NumPy doesn't have the HBM-bandwidth bottleneck that motivates kernel fusion in the first place. Extending the `llvm_kernels.py` approach (or a Triton/CUTLASS backend) to those ops — without touching the IR, fusion pass, or specializer — is the path to measured GPU numbers.
